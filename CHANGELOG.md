...
- **A request could outlive the crawl that owned it.** The backoff was clipped to the remaining deadline, but the retry it then released ran on the fixed 10s socket timeout, which no budget check can interrupt — they run before a request starts and after it ends, never during. A 3s-budget crawl measured 10.7s. Every request is now bounded by `min(10s, remaining maxDurationMs)`, which also fixes the same overrun on the FIRST attempt, where it predates retries.
- **HTTP 429 was reported as a broken link.** It is the one error status that describes our own request rate rather than the resource, so calling it broken blames a site for how hard we crawled it — the exact mistake this release exists to fix, arriving with a status code attached. A 429 is now retried first, honouring `Retry-After` (bounded to 10s and clipped by the crawl deadline), and a target still throttled after every retry is reported `unverified` with `reason: 'throttled'`, never as a finding.
- **`unverified` claimed targets "never answered" when some had.** A redirect loop and an oversized body both end as a null-status `fetch-error`, but both did answer. `reason` is now a typed `CrawlUnverifiedReason` — `unreachable`, `throttled`, `redirect-limit`, `body-too-large`, `unknown` — and each row carries the `statusCode` when there was one. `CrawlPageObservation` gains `errorCode`, the typed code behind the human `error` string, which is what makes the distinction derivable at all.
- **Retry accounting counted requests that were never sent, and lost the failure that prompted them.** `attempted` was incremented before the backoff, so a crawl deadline expiring mid-wait recorded an attempt that never dispatched; and the resulting budget error propagated in place of the original transient one, so `processItem` dropped the page entirely — no page row, no `unverified` entry, nothing to explain the gap. The counter now increments only once a retry can actually be dispatched, and a deadline that expires during backoff re-raises the ORIGINAL failure so the observation survives.

### Changed

- **Crawl schema `1.2` → `2.0` and crawl engine `1.2.0` → `2.0.0`.** A major bump, not `1.3`: the shape change is not additive. `CrawlDeadLinkResult` gains a required `unverified` array, `CrawlDeadLinkFinding` narrows to `statusCode: number` / `reason: 'http-error'`, `CrawlUnverifiedLinkFinding.reason` is a typed union rather than the single literal `'fetch-error'`, and `CrawlPageObservation` gains a required `errorCode`. The `1.1` → `1.2` bump was explicitly additive; this one is not, and versioning it as a minor would tell a consumer the opposite of the truth.
- **Superseded: crawl schema `1.2` → `1.3` and crawl engine `1.2.0` → `1.3.0`.** The report shape changed and the traversal changed. Unlike 1.1 → 1.2 this is NOT purely additive: `CrawlDeadLinkResult` gains a required `unverified` array, and `CrawlDeadLinkFinding` narrows to `statusCode: number` and `reason: 'http-error'`. Reading a finding still compiles (both are narrowings); constructing a `CrawlDeadLinkResult` does not. `unverified` is deliberately required rather than optional — a consumer skipping it is the exact failure this release fixes. Dead-link counts drop against 5.x on any site where the crawler ever failed to connect, so a stored crawl cannot be compared across this boundary.
...
- **BREAKING: `weight` is not a percentage, and we printed it as one.** The core weights sum to 111, not 100, and the score correctly divides by the actual total, so a weight-12 factor is worth about 12.4 percent of a typical page's score. Both formatters printed `${factor.weight}%`, overstating all sixteen and producing a column that never added up; a downstream dashboard reproduced the same mistake independently, which is the signal that the payload was missing the number rather than that consumers were careless. Factors now carry `sharePct`, computed from the same denominator the score uses and allocated by largest remainder so the shares sum to EXACTLY 100 (rounding each weight independently does not: the 16 core factors over a denominator of 111 come to 99.9, which would reproduce the same never-adds-up column one decimal place down). Every factor also carries the RESOLVED `applicable`, not just what an analyzer volunteered, so a page-specific factor judged by the presence fallback no longer comes back as score 0 with no flag, indistinguishable from one that applied and scored zero. It is OPTIONAL so `compare` still reads reports produced before it existed, and the exported `factorSharePct` helper derives it from a report's own weights in that case, never from `weight` alone. A non-applicable factor reports `sharePct: 0`, because it moved the score by nothing. The markdown column header is now "% of score", and `docs/scoring.md` no longer claims the weights sum to 100.

## 4.7.0 (2026-08-11)

### Added

- **Every link records where it sits in the page.** Consumers had no ground truth for telling a nav or footer link from an in-prose editorial one, so they inferred it from ubiquity: key on (target URL, anchor text) and call the link chrome when the pair repeats across most of the site. That inference cannot see the case it most needs to. On canonry.ai, an editorial in-prose link `/blog/how-to-rank-on-chatgpt -> /chatgpt-seo-agency` with the anchor "ChatGPT SEO Agency" is byte-identical to the primary nav's link to the same page, because good anchor text reuses the destination's canonical name — which is the name the nav already uses. The pair repeated on nearly every page, the editorial link was classified as chrome and hidden from the site map, and 53 newly added editorial links moved the measured content-link count by zero. Each `<a href>` occurrence is now classified from HTML landmarks as `navigation`, `content`, or `unknown`, resolved on the NEAREST landmark ancestor so nesting reads correctly: a `nav` inside `main` is navigation. Placement never looks at class names or ids — a `<div class="footer">` with no landmark yields `unknown`, a deliberate absence of evidence rather than the same unreliable guess in a new place, and the consumer owns the policy for it. Detection is one memoized upward walk per link, so a page costs a single visit per node on any link's ancestor path.
- **`header` and `footer` are scoped, so a post's own byline and tags stay content.** Per HTML-ARIA a `header` maps to `banner` and a `footer` to `contentinfo` only when the element is not a descendant of `article`, `aside`, `main`, `nav`, or `section`, or of an element whose role is `article`, `complementary`, `main`, `navigation`, or `region`. The condition is the UNION of the two, so `<div role="main"><header>` is content and `<section role="doc-chapter"><header>` is still scoped. An article's own header (title, byline) and footer (author bio, tags) are the article's content, and calling them chrome would hide editorial links on exactly the pages that matter most. A `header` scoped by a bare `section` resolves to `unknown`, since neither element is a placement landmark. `nav` and `aside` are chrome at any depth; the accessible-name condition HTML-AAM puts on a scoped `aside` is deliberately not applied, because whether a pull-quote is furniture should not depend on whether an author wrote an `aria-label`.
- **The first recognized role wins, landmark or not.** `role` is an ordered fallback list, so the engine takes the first token that is a recognized role and ignores the rest. A placement landmark role gives the placement; a role that is recognized but carries no placement means the element is NOT a landmark and its tag name is not consulted, because an author role overrides native semantics — `<nav role="button navigation">` is a button, and `<nav role="tablist">` is not navigation. Only when no token is a recognized role does the tag name decide. Note the deliberate asymmetry with scoping, where a role ADDS to the tag rather than overriding it: placement asks what an element is, and scoping asks whether a sectioning container encloses the header.
- **The recognized role enumeration is complete, not merely large.** ARIA 1.2 concrete roles, DPUB-ARIA 1.1, and Graphics-ARIA 1.0 are the full set of W3C role modules, so `<footer role="doc-footnote">` is a footnote rather than contentinfo and `<nav role="graphics-document">` is not navigation. Omitting a module silently breaks first-recognized-role ordering by making a valid role look unrecognized. Abstract roles are excluded because authors must not use them and user agents ignore them. Membership is `isRecognizedAriaRole(token)`; `RECOGNIZED_ARIA_ROLES` is a frozen sorted array for enumeration. The live `Set` the resolver reads is deliberately NOT exported: a `Set` cannot be frozen, so exporting it would let a consumer `.add()` a role and change every later placement while `linkPlacementRulesetVersion` still reported the published ruleset.
- **An author role places the same as the native element it mirrors.** `<div role="article">` is content exactly as `<article>` is. Placement, role mapping, and header/footer scoping are all generated from one landmark table, so a rule cannot be added to the tag path and forgotten on the role path. `section` / `role="region"` is in that table as a landmark carrying NO placement: a generic sectioning container says nothing about chrome versus content, so the walk continues past it, but it still scopes a `header`.
...
- **Published constants the engine reads are frozen.** `DEFAULT_SITE_CRAWL_LIMITS`, `DEFAULT_COMPARE_POLICY`, `SPEC_RULES`, and `FACTOR_SPEC_RULES` were exported as live objects that internal logic also reads, so a consumer who mutated one silently changed engine behavior while every version string still reported the published ruleset, making the resulting report unreproducible and the drift invisible. `readonly` and `Readonly<T>` are erased at compile time and never prevented this. All four are now deeply frozen, along with their nested rules and arrays. No shape changed, and reading them is unaffected.

### Changed

- **Crawl schema `1.1` → `1.2` and crawl engine `1.1.0` → `1.2.0`** for the added edge field and the new thing the traversal records. Purely additive: no existing field changed shape or meaning, and a consumer reading the 1.1 contract keeps working untouched. Both new fields are **optional**, so a consumer that constructs a `CrawlEdgeObservation` or `CrawlSummary` still compiles. That is also the truthful model: a graph captured before this ruleset has no placement data, and a consumer has to handle its absence rather than assume the field.
